...
ifeq ($(BR2_TARGET_RUN_TESTSCRIPT),y)
define BUSYBOX_SET_GETTY
	$(SED) '/# GETTY_OPTIONS$$/s~^.*#~OPTIONS="$(SYSTEM_GETTY_OPTIONS)" #~' \
		$(TARGET_DIR)/etc/init.d/S55runtest
	$(SED) '/# GETTY_BAUDRATE$$/s~^.*#~BAUDRATE="$(SYSTEM_GETTY_BAUDRATE)" #~' \
		$(TARGET_DIR)/etc/init.d/S55runtest
...
define BUSYBOX_SET_BOOTCMD
	$(SED) '/# REBOOT_MESSAGE$$/s~^.*#~MESSAGE="$(SYSTEM_SHUTDOWN_MESSAGE)" #~' \
		$(TARGET_DIR)/run.sh
	$(SED) '/# REBOOT_COMMAND$$/s~^.*#~COMMAND="$(SYSTEM_SHUTDOWN_COMMAND)" #~' \
		$(TARGET_DIR)/run.sh
endef
BUSYBOX_TARGET_FINALIZE_HOOKS += BUSYBOX_SET_BOOTCMD
else
define BUSYBOX_SET_GETTY
...
endef
endif # BR2_TARGET_RUN_TESTSCRIPT
BUSYBOX_TARGET_FINALIZE_HOOKS += BUSYBOX_SET_GETTY
endif # BR2_TARGET_GENERIC_GETTY

ifeq ($(BR2_ROOTFS_DEVICE_CREATION_DYNAMIC_DEVTMPFS),y)
define BUSYBOX_SET_DEVTMPFS
	$(SED) '/# DEVTMPFS$$/s~^#~~' $(TARGET_DIR)/etc/inittab
endef
BUSYBOX_TARGET_FINALIZE_HOOKS += BUSYBOX_SET_DEVTMPFS
endif
...
ifeq ($(BR2_TARGET_RUN_TESTSCRIPT),y)
define BUSYBOX_INSTALL_TESTSCRIPTS
	$(INSTALL) -D -m 0755 package/busybox/run.sh $(TARGET_DIR)/run.sh
	$(INSTALL) -D -m 0755 package/busybox/S55runtest $(TARGET_DIR)/etc/init.d/S55runtest
endef
endif # BR2_TARGET_RUN_TESTSCRIPT
